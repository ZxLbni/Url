...
<body>
  ...
  <a href="https://lustorlove.co" target="_self">Follow Me</a>
</body>`
...
async function handleRequest(request) {
  ...
  if(!path){
    ...
    const html= await fetch(""+config.theme+"/index.html")
  ...
  if (location) {
    if (config.safe_browsing_api_key){
      if(!(await is_url_safe(location))){
        let warning_page = await fetch("https://zxlbni.github.io/Url/safe-browsing.html")
    ...
    if (config.no_ref=="on"){
      let no_ref= await fetch("https://zxlbni.github.io/Url/no-ref.html")
